...
import { User, LogOut, Star, Bell, Save, Key, Shield, UserCog } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    setFeedback(null);
    try {
      await fetchApi("/v1/profile/me", { method: "PATCH", body: JSON.stringify(profileForm) });
          ...
          <div className="md:col-span-1 bg-blanc border border-champagne/30 rounded-2xl p-5 sm:p-6 shadow-sm md:sticky md:top-24 space-y-5">
            <div className="flex md:flex-col items-center md:text-center gap-4 md:gap-0">
              ...
              <div className="min-w-0 flex-1 md:w-full">
                ...
                {isStaff && <Badge className="mt-2 bg-or text-noir font-poppins font-black text-[10px] px-2 py-0.5 border-0 inline-block">{getRoleBadge(user.role)}</Badge>}
              </div>
            </div>
            ...
            <nav className="flex md:flex-col gap-1.5 overflow-x-auto pb-1 md:pb-0">
              <button onClick={() => setActiveTab("public")} className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl font-montserrat font-bold text-xs transition-all text-left whitespace-nowrap shrink-0 md:shrink md:w-full ${activeTab === "public" ? "bg-or text-noir shadow-sm" : "text-champagne hover:text-noir hover:bg-champagne/10"}`}><UserCog size={15} /> Identité</button>
              <button onClick={() => setActiveTab("security")} className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl font-montserrat font-bold text-xs transition-all text-left whitespace-nowrap shrink-0 md:shrink md:w-full ${activeTab === "security" ? "bg-or text-noir shadow-sm" : "text-champagne hover:text-noir hover:bg-champagne/10"}`}><Shield size={15} /> Sécurité</button>
              <button onClick={() => setActiveTab("preferences")} className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl font-montserrat font-bold text-xs transition-all text-left whitespace-nowrap shrink-0 md:shrink md:w-full ${activeTab === "preferences" ? "bg-or text-noir shadow-sm" : "text-champagne hover:text-noir hover:bg-champagne/10"}`}><Star size={15} /> Abonnements</button>
            </nav>
              ...
              <form onSubmit={handleProfileSubmit} className="bg-blanc border border-champagne/30 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6 animate-in fade-in duration-300">
                ...
                <div className="space-y-2">
                  <Label className="font-montserrat font-bold text-xs text-champagne uppercase flex justify-between">
                    <span>Biographie (Auteurs)</span><span className="font-normal opacity-70">{profileForm.bio.length}/250</span>
                  </Label>
                  <Textarea maxLength={250} rows={3} value={profileForm.bio} onChange={(e) => setProfileForm({ ...profileForm, bio: e.target.value })} placeholder="Décrivez votre activité..." className="border-champagne/40 rounded-xl bg-blanc focus-visible:ring-or/30 focus-visible:border-or resize-none text-sm" />
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-4">
                    {avatars.map((avatar) => (
                      <button key={avatar.id} type="button" onClick={() => setProfileForm({ ...profileForm, avatar_ref: avatar.id })} className={`w-10 h-10 rounded-full border-2 transition-all ${avatar.color} ${profileForm.avatar_ref === avatar.id ? "border-or scale-110 shadow-md" : "border-transparent opacity-60 hover:opacity-100"}`} aria-label={`Sélectionner l'avatar ${avatar.label}`} />
                    ))}
                  </div>
                </div>
              </form>
              ...
              <form onSubmit={handleSecuritySubmit} className="bg-blanc border border-red-200 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6 relative overflow-hidden animate-in fade-in duration-300">
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  <div className="space-y-2">
                    ...
                    <Input type="password" value={securityForm.newPassword} onChange={(e) => setSecurityForm({ ...securityForm, newPassword: e.target.value })} placeholder="Min. 14 caractères" className="border-champagne/40 rounded-xl bg-blanc focus-visible:ring-or/30 focus-visible:border-or text-sm py-5" />
                  </div>
                  <div className="space-y-2">
