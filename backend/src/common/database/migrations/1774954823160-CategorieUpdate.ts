...
export class CategorieUpdate1774954823160 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`category\` ADD \`icon\` varchar(10) NULL`);
        await queryRunner.query(`ALTER TABLE \`category\` ADD \`image_bandeau_url\` varchar(255) NOT NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`declaration_objet\` ADD CONSTRAINT \`FK_fa23c683df53e3f2dbd33d3370e\` FOREIGN KEY (\`user_id\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`category\` DROP COLUMN \`image_bandeau_url\``);
