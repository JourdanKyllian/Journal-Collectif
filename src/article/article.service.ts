...
export class ArticleService {
  ...
  async create(createArticleDto: CreateArticleDto, userId: number, userRole: string) {
    if (!userId) throw new ForbiddenException("Authentification invalide.");

    // 1. Vérification profil complet
    const user = await this.usersRepository.findOne({ where: { id: userId } });
    if (!user || !user.is_phone_verified || !user.firstname || !user.lastname) {
      throw new ForbiddenException("Profil incomplet (téléphone, nom, prénom requis).");
    }

    // 2. Vérification catégorie
    const category = await this.categoryRepository.findOne({ where: { id: createArticleDto.categoryId } });
    if (!category) throw new NotFoundException('Catégorie introuvable');

    // 3. Gestion intelligente du statut
    let statutFinal = createArticleDto.statut as ArticleStatus || ArticleStatus.BROUILLON;
    ...
    await this.auteurArticleRepository.save(
      this.auteurArticleRepository.create({ article: savedArticle, user: user })
    );
  ...
  async update(id: number, updateArticleDto: UpdateArticleDto, userId: number, userRole: string) {
    ...
    // Sécurité : Seul l'auteur ou un Admin/Modo peut modifier
    ...
    if (updateArticleDto.statut === ArticleStatus.PUBLIE && article.statut !== ArticleStatus.PUBLIE) {
